...
CONSTRAINT("Exactly-2(%s)" % ",".join(srange("x", N)))
...
for m in range(1, N//2 + 1):
  ...
  OUTCOME("lighter", "((%s) & !(%s) & !y) | (!(%s) & (%s) & y)" % (params(1, m), params(m+1, 2*m), params(1, m), params(m+1, 2*m)))
  OUTCOME("heavier", "((%s) & !(%s) & y) | (!(%s) & (%s) & !y)" % (params(1, m), params(m+1, 2*m), params(1, m), params(m+1, 2*m)))

  # both sides weight the same
  OUTCOME("same", "!(%s) | ((%s) & (%s))" % (params(1, 2*m), params(1, m), params(m+1, 2*m)))
